...
1. Approving new projects with appropriate details, as appropriate
2. Approving recommendations from the working groups, as appropriate

#### JC Voting Membership {.body}

Voting members of the JC are appointed by the SDOs with six members appointed by each of the three SDOs, for a total of 18 voting member positions. It is the responsibility of the SDOs to coordinate among each other to ensure balance among these members.

!!! question
    Do we allow alternates, and if so, is it by company?

#### JC Observer Membership {.body}

!!! question
    It seems to me that this does not formally exist at present, but I think this is the practice, yes?

Observer membership to the JC is open to any party with a vested interest in ITS that contacts the NTCIP Coordinator to have their name added to the membership list. The NTCIP Coordinator has the discretion to remove observing members from the list that have not participated in recent meetings.

#### Quorum {.body}

Quorum is established by a simple majority of currently appointed members.

#### Voting {.body}

Unless otherwise stated in detailed processes, decisions by the JC are made by simple majority of those voting (i.e., more than half of the votes cast) when a quorum is available. Voting by correspondence is allowed and abstentions are considered in quorum determination but are not considered in the simple majority determination.
...
Membership within each NTCIP group  is by organization. For each WG, each member is assigned to one of the following membership categories:
    ...
    - NOTE: Membership categories can vary by WG (e.g., an ASC manufacturer would be a manufacturer on the ASC WG but could be general interest for the RSU WG if they do not manufacturer RSUs)

Membership in AGs and AHGs default to the same rules as WGs but, because they do not directly work on products for public consumption, can be more flexible at the direction of the JC, as deemed appropriate. For example, an ad-hoc group focused on gaining feedback from public agencies regarding education needs could be limited the user category.

#### Quorum {.body}

To establish a quorum, a group will have representation from a minimum of two members from each membership category applicable to the group. A group that does not have, on its roster, at least two members from each applicable membership category will be considered inactive.
...
!!! question
    Given the above proposed rules, do we need to say anything about:

    - Lapsing membership (e.g., due to inactivity)
    - Reinstating membership
    - Inactive groups

#### Reactivating groups {.body}

Upon initial establishment and whenever a dormant group is reactivated, the NTCIP Coordinator is responsible for publicizing the action to promote membership within the group.

### TC Chair {.body}

The TC chair is appointed by the SDOs and is responsible for:
...
Membership within any group (exclusive of the JC) is open to any organization with an vested interest in ITS. Each organization shall designate a primary representative to cast its votes; alternates can be designated as well to ensure representation when conflicts arise. Organizations can elect to be observers, in which case, they are not included in votes.

Participants are responsible for participating in meetings, participating in discussions within the group, casting votes, and informing the NTCIP Coordinator of any changes to their contact or employment information.
...
Every NTCIP document (draft or published) is assigned a primary editor by the NTCIP Coordinator. The editor is responsible for consolidating all of the comments received to date into a coherent document that conforms with the NTCIP format as defined in [NTCIP 8002](https://ite-org.github.io/NTCIP-8002/). The editor is also responsible for managing written comments received, formulating draft responses, facilitating discussions on the comments and draft responses, and submitting the final comments with resolutions to the NTCIP Coordinator. Finally, the editor is responsible for developing the complete standards development report package for submittal to the SDOs prior to ballot.

For online development projects, the editor is also responsible for the tasks assigned to the Maintainer in [NTCIP 8008](https://ite-org.github.io/NTCIP-8008/).
...
The NTCIP has three approval processes resulting in different levels of standardization as presented below. The three processes mainly differ in the number of iterations that are required and who approves the final document, whereas the [Steps](#Steps-body) required within the process are consistent among the processes.
